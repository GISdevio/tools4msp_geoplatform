...
const uploadDataset = async (layer: V3Layer): Promise<{ id: string; title: string }> => {
  const filesPath = `${dataDirPath}/vector-layers/download/${layer.id}`

  const dbfFilePath = globSync(`${filesPath}/*.dbf`)
  if (dbfFilePath.length !== 1) { throw new Error(`Unexpected number ${dbfFilePath.length} of "dbf" files found in path`) }

  const prjFilePath = globSync(`${filesPath}/*.prj`)
  if (prjFilePath.length !== 1) { throw new Error(`Unexpected number ${prjFilePath.length} of "prj" files found in path`) }

  const shpFilePath = globSync(`${filesPath}/*.shp`)
  if (shpFilePath.length !== 1) { throw new Error(`Unexpected number ${shpFilePath.length} of "shp" files found in path`) }

  const shxFilePath = globSync(`${filesPath}/*.shx`)
  if (shxFilePath.length !== 1) { throw new Error(`Unexpected number ${shxFilePath.length} of "shx" files found in path`) }

  const dbfFile = await readFile(dbfFilePath[0]!) as Blob
  const prjFile = await readFile(prjFilePath[0]!) as Blob
  const shpFile = await readFile(shpFilePath[0]!) as Blob
  const shxFile = await readFile(shxFilePath[0]!) as Blob

  const formdata = new FormData()
  formdata.append('base_file', shpFile, path.basename(shpFilePath[0]!))
  formdata.append('charset', 'UTF-8')
  formdata.append('store_spatial_files', 'true')
  formdata.append('time', 'true')
  formdata.append('shp_file', shpFile, path.basename(shpFilePath[0]!))
  formdata.append('prj_file', prjFile, path.basename(prjFilePath[0]!))
  formdata.append('dbf_file', dbfFile, path.basename(dbfFilePath[0]!))
  formdata.append('shx_file', shxFile, path.basename(shxFilePath[0]!))
...
const uploadStyle = async (layer: V3Layer, datasetTitle: string) => {
  if (!layer.stylePath) {
    logger.warn('The layer has no style')
    return
  }
...
const createLayers = async () => {
  ...
  try {
    creationStateMap = await readJson<CreationStateMap>('vector-layers/creation-state')
  } catch {
  ...
  const reportPath = `vector-layers/create-report/${dayjs().format('YYYY-MM-DD HH:mm:ss')}`
  ...
  const dumpResults = async () => {
    await dumpJson('vector-layers/creation-state', creationStateMap)

    const report = `# Created vector layers
...
`
  ...
  logger.log('\n')
  logger.success('Created vector layers')
}
