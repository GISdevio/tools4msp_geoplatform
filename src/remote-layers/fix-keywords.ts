...
const main = async () => {
  const platformsDiff = await readJson<PlatformsDiff>('remote-layers/platforms-diff')
  ...
  try {
    report = await readJson<Report>('remote-layers/fix-keyword-state')
  } catch {
  ...
  for (const [idx, layersMap] of platformsDiff.layersInBothPlatforms.entries()) {
    ...
    await dumpJson('remote-layers/fix-keyword-state', report)
  }
}
