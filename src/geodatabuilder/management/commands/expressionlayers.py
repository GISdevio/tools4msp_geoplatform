...
from geonode.layers.models import Dataset, LayerFile
...
ALLOWED_EXTENSION = ['.geotiff', '.tif', '.shp']
...
def getFileToId(resourceid, directory_temp, grid, res, attribute):
    instance = Dataset.objects.filter(id=resourceid).first()
    if instance:
        try:
            ...
            for l in layer_files:
                if l.file:
                    geonode_layer_path = l.file.path
                    ...
                    if(not instance.is_vector() or (instance.is_vector() and original_ext.lower() == '.shp')):
                        if instance.is_vector():
                            ...
                            if result_rasterize.decode('utf-8').find('100 - done') != -1:
                        ...
                        if (grid):
                            ...
                            if result.decode('utf-8').find('100 - done') != -1:
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        result_tif = 'result_'+filename+'.tif'
