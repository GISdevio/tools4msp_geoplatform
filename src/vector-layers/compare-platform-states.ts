...
const main = async () => {
  ...
  const oldLayers = await readJson<OldLayers>('vector-layers/layers')

  type NewLayers = { meta: { downloadedAt: string }; objects: { pk: string; title: string }[] }
  const newLayers = await readJson<NewLayers>('vector-layers/new-layers')

  type CreationState = Record<string, { newId: string }>
  const creationState = await readJson<CreationState>('vector-layers/creation-state')
  ...
  await dumpJson('vector-layers/platforms-diff', platformsDiff)
}
