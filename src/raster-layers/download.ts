...
const downloadFiles = async (layer: Layer): Promise<DownloadFilesResult> => {
  ...
  const filesPath = `raster-layers/download/${layer.id}`
  ...
  try {
    ...
    const sldFilePaths = globSync(`${fullPath}/*.sld`)

    let stylesCount = 0

    for (const sldFilePath of sldFilePaths) {
...
const downloadRasterLayers = async () => {
  logger.info('Downloading raster layers...\n')
  ...
  try {
    downloadState = await readJson<DownloadState>('raster-layers/download-state')
  } catch {
  ...
  let data: V3Layer[] = []
  try {
    data = await readJson<V3Layer[]>('raster-layers/data')
  } catch {
    logger.warn('No existing data')
  }

  // const layersRes = await v3Client.getJson<LayersResponse>('/api/layers/?limit=1000&offset=0&order_by=title&type__in=raster')
  const layersRes = await readJson<LayersResponse>('raster-layers/layers')
  ...
  const reportPath = `raster-layers/download-report/${dayjs().format('YYYY-MM-DD HH:mm:ss')}`

  const dumpResults = async () => {
    await dumpJson('raster-layers/data', data)
    await dumpJson('raster-layers/download-state', downloadState)

    const report = `# Downloaded raster layers
...
`
  ...
  logger.log('\n')
  logger.success('Downloaded raster layers')
}

export default downloadRasterLayers
