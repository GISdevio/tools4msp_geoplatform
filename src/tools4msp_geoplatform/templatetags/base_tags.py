...
@register.inclusion_tag(filename="base/home_counters.html", takes_context=True)
def show_resource_counters(context):
    """Return total counts of layers, maps, documents, and users."""
    request = context['request']
    
    layers_count = Dataset.objects.count()
    maps_count = Map.objects.count()
    documents_count = Document.objects.count()
    # Exclude the anonymous user so it doesn't get counted
    users_count = get_user_model().objects.exclude(username='AnonymousUser').count()
